/*
 * contrib/ora_btree_gist/btree_varcharchar.c
 */
#include "postgres.h"
...
Datum
gbt_varcharchar_compress(PG_FUNCTION_ARGS)
{
...
Datum
gbt_varcharchar_consistent(PG_FUNCTION_ARGS)
{
	GISTENTRY  *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	void	   *query = (void *) DatumGetTextP(PG_GETARG_DATUM(1));
	StrategyNumber strategy = (StrategyNumber) PG_GETARG_UINT16(2);
