...
-- IvorySQL: in Oracle-compatible mode an empty-string literal is lexed as
-- NULL (ivorysql.enable_emptystring_to_NULL), which would make the STRICT
-- pgp_pub_decrypt calls below that pass an empty passphrase return NULL
-- instead of running.  Disable the conversion so the empty-string
-- passphrase behaves as in PostgreSQL.
