...
SELECT (time_after - time_before) <= '10 minutes'::pg_catalog.interval, time_after >= time_before
FROM (SELECT current_setting('test.time_after')::pg_catalog.timestamptz AS time_after, (SELECT current_setting('test.time_before')::pg_catalog.timestamptz) AS time_before) AS d;
