...
MERGE INTO replication_example t
	...
	ON t.id = s.id
	WHEN MATCHED THEN
		UPDATE SET somenum = somenum + 1 where t.id < 0
...
MERGE INTO replication_example t
	...
	ON t.id = s.id
	WHEN MATCHED THEN
		UPDATE SET somenum = somenum where t.id >= 0
		DELETE where t.id >= 0;
...
-- check toast support
BEGIN;
CREATE SEQUENCE toasttable_rand_seq START WITH 79 INCREMENT BY 1499; -- portable "random"
