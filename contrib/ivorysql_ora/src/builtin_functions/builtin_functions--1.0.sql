...
DECLARE
BEGIN
	RETURN sys.dbms_application_info_get_client_info();
END;
$$ LANGUAGE plpgsql STRICT SET client_min_messages to error;
...
BEGIN
	IF upper(a) = 'USERENV' THEN
	  CASE upper(b)
		  ...
		  WHEN 'ACTION' THEN
			SELECT sys.dbms_application_info_get_action() INTO res;
		  ...
		  WHEN 'MODULE' THEN
			SELECT sys.dbms_application_info_get_module() INTO res;
