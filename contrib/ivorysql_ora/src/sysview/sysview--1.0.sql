...
SELECT
    ...
    FORMAT_TYPE(PG_ATTRIBUTE.ATTTYPID, PG_ATTRIBUTE.ATTTYPMOD)::VARCHAR2(128) AS DATA_TYPE,
    NULL::VARCHAR2(3) AS DATA_TYPE_MOD,
    SYS.ORA_CASE_TRANS(PG_NAMESPACE.NSPNAME::VARCHAR2)::VARCHAR2(128) AS DATA_TYPE_OWNER,
    PG_ATTRIBUTE.ATTLEN::NUMBER AS DATA_LENGTH,
    CASE
        WHEN PG_ATTRIBUTE.ATTTYPID = 21 THEN 5
        WHEN PG_ATTRIBUTE.ATTTYPID = 23 THEN 10
        WHEN PG_ATTRIBUTE.ATTTYPID = 20 THEN 19
        WHEN PG_ATTRIBUTE.ATTTYPID = 700 THEN 63
        WHEN PG_ATTRIBUTE.ATTTYPID = 701 THEN 126
        WHEN PG_ATTRIBUTE.ATTTYPID = 'pg_catalog.numeric'::REGTYPE
             AND PG_ATTRIBUTE.ATTTYPMOD > 4
             THEN ((PG_ATTRIBUTE.ATTTYPMOD - 4) >> 16) & 65535
        ELSE NULL
    END::NUMBER AS DATA_PRECISION,
    CASE
        WHEN PG_ATTRIBUTE.ATTTYPID = 'pg_catalog.numeric'::REGTYPE
        ...
        WHEN PG_ATTRIBUTE.ATTTYPID IN (20, 21, 23) THEN 0
        ELSE NULL
    END::NUMBER AS DATA_SCALE,
    CASE WHEN PG_ATTRIBUTE.ATTNOTNULL THEN 'N' ELSE 'Y' END::VARCHAR2(1) AS NULLABLE,
    pg_get_expr((SELECT adbin FROM pg_attrdef WHERE adrelid = PG_ATTRIBUTE.ATTRELID AND adnum = PG_ATTRIBUTE.ATTNUM), PG_ATTRIBUTE.ATTRELID)::VARCHAR2(4000) AS COLUMN_DEFAULT,
    (SELECT pg_get_expr(adbin, adrelid) FROM pg_attrdef WHERE adrelid = PG_ATTRIBUTE.ATTRELID AND adnum = PG_ATTRIBUTE.ATTNUM)::TEXT AS DATA_DEFAULT,
    0::NUMERIC AS NUM_DISTINCT,
    NULL::RAW AS LOW_VALUE,
    NULL::RAW AS HIGH_VALUE,
    0::NUMBER AS DENSITY,
    SUM(CASE WHEN NOT PG_ATTRIBUTE.ATTNOTNULL THEN 1 ELSE 0 END) OVER (PARTITION BY PG_CLASS.OID)::NUMERIC AS NUM_NULLS,
    0::NUMBER AS NUM_BUCKETS,
    TO_TIMESTAMP('1970-01-01', 'YYYY-MM-DD') AS LAST_ANALYZED,
    0::NUMERIC AS SAMPLE_SIZE,
    (SELECT pg_encoding_to_char(encoding) FROM pg_database WHERE datname = current_database()) AS CHARACTER_SET_NAME,
    CASE
    ...
    AVG(OCTET_LENGTH(PG_ATTRIBUTE.ATTNAME::TEXT)) OVER (PARTITION BY PG_CLASS.OID)::NUMERIC AS AVG_COL_LEN,
    CASE
        ...
        ELSE NULL
    END::NUMBER AS CHAR_LENGTH,
    CASE WHEN PG_TYPE.TYPNAME = ANY(ARRAY['bpchar','char','nchar']) THEN 'B' ELSE NULL END::VARCHAR2(1) AS CHAR_USED,
    'NO'::VARCHAR2(10) AS V80_FMT_IMAGE,
    'NO'::VARCHAR2(10) AS DATA_UPGRADED,
    'NONE'::VARCHAR2(10) AS HISTOGRAM,
    OCTET_LENGTH(pg_get_expr((SELECT adbin FROM pg_attrdef WHERE adrelid = PG_ATTRIBUTE.ATTRELID AND adnum = PG_ATTRIBUTE.ATTNUM), PG_ATTRIBUTE.ATTRELID)::TEXT)::NUMERIC AS DEFAULT_LENGTH,
    0::NUMBER AS NULLABLE_COLUMN_ID,
    NULL::VARCHAR2(128) AS COLLATION_NAME,
    NULL::VARCHAR2(128) AS COLLATION_SCHEMA,
    NULL::VARCHAR2(128) AS COLLATION_OWNER,
    ...
    NULL::NUMBER AS LOB_LENGTH,
    'NO'::VARCHAR2(10) AS NESTED,
    CASE WHEN PG_ATTRIBUTE.ATTISDROPPED OR PG_ATTRIBUTE.ATTNUM < 0 THEN 'YES' ELSE 'NO' END::VARCHAR2(10) AS HIDDEN_COLUMN,
    CASE WHEN PG_ATTRIBUTE.ATTGENERATED <> '' THEN 'YES' ELSE 'NO' END::VARCHAR2(10) AS VIRTUAL_COLUMN,
    0::NUMBER AS SEGMENT_ID,
