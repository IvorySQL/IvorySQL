...
-- Oracle-compatible index metadata views.  INDEX_TYPE reports
-- FUNCTION-BASED NORMAL for expression indexes and for indexes with
-- descending key columns, NORMAL otherwise; GENERATED and
-- CONSTRAINT_INDEX mark constraint-backing indexes (pg_constraint
-- conindid), and STATUS reports UNUSABLE for indexes disabled via
-- ALTER INDEX ... UNUSABLE (pg_index.indisunusable).  Storage
-- columns without a PostgreSQL counterpart are NULL, B-tree
-- statistics (BLEVEL, LEAF_BLOCKS, ...) are not tracked and stay
-- NULL; NUM_ROWS comes from the index relation's reltuples (exact
-- when CREATE INDEX built it, 0 for indexes created before rows
-- were inserted until ANALYZE refreshes them).

CREATE OR REPLACE VIEW SYS.DBA_INDEXES AS
SELECT
	SYS.ORA_CASE_TRANS(PG_GET_USERBYID(I.RELOWNER)::VARCHAR2)::VARCHAR2(128) AS OWNER,
	SYS.ORA_CASE_TRANS(I.RELNAME::VARCHAR2)::VARCHAR2(128) AS INDEX_NAME,
	CASE WHEN X.INDEXPRS IS NOT NULL OR EXISTS (SELECT 1 FROM UNNEST(X.INDOPTION) AS O WHERE O & 1 <> 0) THEN 'FUNCTION-BASED NORMAL' ELSE 'NORMAL' END::VARCHAR2(21) AS INDEX_TYPE,
	NULL::VARCHAR2(30) AS INDEX_SUBTYPE,
	SYS.ORA_CASE_TRANS(PG_GET_USERBYID(T.RELOWNER)::VARCHAR2)::VARCHAR2(128) AS TABLE_OWNER,
	SYS.ORA_CASE_TRANS(T.RELNAME::VARCHAR2)::VARCHAR2(128) AS TABLE_NAME,
	'TABLE'::VARCHAR2(11) AS TABLE_TYPE,
	CASE X.INDISUNIQUE WHEN true THEN 'UNIQUE' ELSE 'NONUNIQUE' END::VARCHAR2(9) AS UNIQUENESS,
	'DISABLED'::VARCHAR2(8) AS COMPRESSION,
	NULL::NUMBER AS PREFIX_LENGTH,
	(SELECT SYS.ORA_CASE_TRANS(S.SPCNAME::VARCHAR2)::VARCHAR2(30) FROM PG_TABLESPACE S WHERE S.OID = I.RELTABLESPACE AND I.RELTABLESPACE <> 0) AS TABLESPACE_NAME,
	...
	NULL::NUMBER AS PCT_THRESHOLD,
	NULL::NUMBER AS INCLUDE_COLUMN,
	NULL::NUMBER AS FREELISTS,
	NULL::NUMBER AS FREELIST_GROUPS,
	NULL::NUMBER AS PCT_FREE,
	CASE I.RELPERSISTENCE WHEN 'u' THEN 'NOLOGGING' ELSE 'LOGGING' END::VARCHAR2(9) AS LOGGING,
	NULL::NUMBER AS BLEVEL,
	NULL::NUMBER AS LEAF_BLOCKS,
	NULL::NUMBER AS DISTINCT_KEYS,
	NULL::NUMBER AS AVG_LEAF_BLOCKS_PER_KEY,
	NULL::NUMBER AS AVG_DATA_BLOCKS_PER_KEY,
	NULL::NUMBER AS CLUSTERING_FACTOR,
	CASE WHEN X.INDISUNUSABLE OR NOT X.INDISVALID THEN 'UNUSABLE' ELSE 'VALID' END::VARCHAR2(10) AS STATUS,
	CASE WHEN I.RELTUPLES >= 0 THEN I.RELTUPLES::NUMBER END AS NUM_ROWS,
	NULL::NUMBER AS SAMPLE_SIZE,
	NULL::DATE AS LAST_ANALYZED,
	'1'::VARCHAR2(10) AS DEGREE,
	'1'::VARCHAR2(10) AS INSTANCES,
	CASE WHEN T.RELKIND = 'p' THEN 'YES' ELSE 'NO' END::VARCHAR2(3) AS PARTITIONED,
	'N'::VARCHAR2(1) AS TEMPORARY,
	CASE WHEN EXISTS (SELECT 1 FROM PG_CONSTRAINT CN WHERE CN.CONINDID = I.OID) THEN 'Y' ELSE 'N' END::VARCHAR2(1) AS GENERATED,
	...
	NULL::NUMBER AS PCT_DIRECT_ACCESS,
	...
	CASE WHEN X.INDEXPRS IS NOT NULL OR EXISTS (SELECT 1 FROM UNNEST(X.INDOPTION) AS O WHERE O & 1 <> 0) THEN 'ENABLED' END::VARCHAR2(8) AS FUNCIDX_STATUS,
	'NO'::VARCHAR2(3) AS JOIN_INDEX,
	NULL::VARCHAR2(3) AS IOT_REDUNDANT_PKEY_ELIM,
	'NO'::VARCHAR2(3) AS DROPPED,
	'VISIBLE'::VARCHAR2(9) AS VISIBILITY,
	NULL::VARCHAR2(14) AS DOMIDX_MANAGEMENT,
	'YES'::VARCHAR2(3) AS SEGMENT_CREATED,
	NULL::VARCHAR2(3) AS ORPHANED_ENTRIES,
	'FULL'::VARCHAR2(7) AS INDEXING,
	'NO'::VARCHAR2(3) AS AUTO,
	CASE WHEN EXISTS (SELECT 1 FROM PG_CONSTRAINT CN WHERE CN.CONINDID = I.OID) THEN 'YES' ELSE 'NO' END::VARCHAR2(3) AS CONSTRAINT_INDEX
FROM PG_CLASS I
JOIN PG_INDEX X
...
GRANT SELECT ON SYS.DBA_INDEXES TO PUBLIC;
...
AND N.NSPNAME NOT IN ('pg_catalog', 'information_schema', 'pg_toast')
AND N.NSPNAME NOT LIKE 'pg\_temp%' ESCAPE '\'
AND PG_GET_USERBYID(I.RELOWNER) = CURRENT_USER;
