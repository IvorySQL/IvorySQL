...
-- Oracle-compatible partition metadata views.  Each row is one
-- partition of a partitioned table (pg_inherits child).  HIGH_VALUE
-- carries the partition bound as pg_get_expr text (e.g. FOR VALUES
-- FROM ... TO ...), which differs from Oracle's bare-literal LONG
-- rendering; PARTITION_POSITION follows pg_inherits order, matching
-- Oracle for partitions created in bound order.  A partition that
-- is itself partitioned reports COMPOSITE = YES with its immediate
-- children as SUBPARTITION_COUNT (PostgreSQL models subpartitioning
-- as nested partitioned tables).
...
SELECT
	...
	CASE WHEN EXISTS (SELECT 1 FROM PG_INHERITS H2 WHERE H2.INHPARENT = P.OID) THEN 'YES' ELSE 'NO' END::VARCHAR2(3) AS COMPOSITE,
	SYS.ORA_CASE_TRANS(P.RELNAME::VARCHAR2)::VARCHAR2(128) AS PARTITION_NAME,
	(SELECT COUNT(*) FROM PG_INHERITS H2 WHERE H2.INHPARENT = P.OID)::NUMBER AS SUBPARTITION_COUNT,
	PG_GET_EXPR(P.RELPARTBOUND, P.OID)::CLOB AS HIGH_VALUE,
	LENGTH(PG_GET_EXPR(P.RELPARTBOUND, P.OID))::NUMBER AS HIGH_VALUE_LENGTH,
	ROW_NUMBER() OVER (PARTITION BY T.OID ORDER BY H.INHSEQNO)::NUMBER AS PARTITION_POSITION,
	(SELECT SYS.ORA_CASE_TRANS(S.SPCNAME::VARCHAR2)::VARCHAR2(30) FROM PG_TABLESPACE S WHERE S.OID = P.RELTABLESPACE AND P.RELTABLESPACE <> 0) AS TABLESPACE_NAME,
	...
	CASE P.RELPERSISTENCE WHEN 'u' THEN 'NOLOGGING' ELSE 'LOGGING' END::VARCHAR2(9) AS LOGGING,
	'DISABLED'::VARCHAR2(8) AS COMPRESSION,
	NULL::VARCHAR2(30) AS COMPRESS_FOR,
	CASE WHEN P.RELTUPLES >= 0 THEN P.RELTUPLES::NUMBER END AS NUM_ROWS,
	CASE WHEN P.RELTUPLES >= 0 THEN P.RELPAGES::NUMBER END AS BLOCKS,
	...
	GREATEST(SP.LAST_ANALYZE, SP.LAST_AUTOANALYZE)::SYS.ORATIMESTAMPTZ::SYS.ORADATE AS LAST_ANALYZED,
	...
	CASE WHEN P.RELFILENODE <> 0 THEN 'YES' ELSE 'NO' END::VARCHAR2(3) AS SEGMENT_CREATED,
	'ON'::VARCHAR2(2) AS INDEXING,
	...
	PG_GET_EXPR(P.RELPARTBOUND, P.OID)::CLOB AS HIGH_VALUE_CLOB,
	NULL::CLOB AS HIGH_VALUE_JSON,
	NULL::NUMBER AS SHARDED_TABLE_CHUNK_ID
FROM PG_INHERITS H
JOIN PG_CLASS P
ON P.OID = H.INHRELID
JOIN PG_CLASS T
ON T.OID = H.INHPARENT
JOIN PG_NAMESPACE N
ON N.OID = P.RELNAMESPACE
LEFT JOIN PG_STAT_USER_TABLES SP
ON SP.RELID = P.OID
WHERE T.RELKIND = 'p'
...
GRANT SELECT ON SYS.DBA_TAB_PARTITIONS TO PUBLIC;
...
AND N.NSPNAME NOT IN ('pg_catalog', 'information_schema', 'pg_toast')
AND N.NSPNAME NOT LIKE 'pg\_temp%' ESCAPE '\'
AND PG_GET_USERBYID(P.RELOWNER) = CURRENT_USER;
