...
SELECT
    ...
    ATT.ATTNUM::NUMBER AS COLUMN_ID,
    FORMAT_TYPE(ATT.ATTTYPID, ATT.ATTTYPMOD)::VARCHAR2(128) AS DATA_TYPE,
    NULL::VARCHAR2(3) AS DATA_TYPE_MOD,
    NULL::VARCHAR2(128) AS DATA_TYPE_OWNER,
    ATT.ATTLEN::NUMBER AS DATA_LENGTH,
    CASE
        WHEN ATT.ATTTYPID = 21 THEN 5
        WHEN ATT.ATTTYPID = 23 THEN 10
        WHEN ATT.ATTTYPID = 20 THEN 19
        WHEN ATT.ATTTYPID = 700 THEN 63
        WHEN ATT.ATTTYPID = 701 THEN 126
        WHEN ATT.ATTTYPID = 'pg_catalog.numeric'::REGTYPE
             AND ATT.ATTTYPMOD > 4
    ...
    CASE
        ...
        WHEN ATT.ATTTYPID IN (20, 21, 23) THEN 0
        ELSE NULL
    END::NUMBER AS DATA_SCALE,
    CASE WHEN ATT.ATTNOTNULL THEN 'N' ELSE 'Y' END::VARCHAR2(1) AS NULLABLE,
    NULL::VARCHAR2(4000) AS COLUMN_DEFAULT,
    NULL::VARCHAR2(4000) AS DATA_DEFAULT,
    NULL::NUMBER AS NUM_DISTINCT,
    NULL::RAW AS LOW_VALUE,
    NULL::RAW AS HIGH_VALUE,
    NULL::NUMBER AS DENSITY,
    NULL::NUMBER AS NUM_NULLS,
    NULL::NUMBER AS NUM_BUCKETS,
    NULL::DATE AS LAST_ANALYZED,
    NULL::NUMBER AS SAMPLE_SIZE,
    NULL::VARCHAR2(30) AS CHARACTER_SET_NAME,
    CASE
        WHEN T.TYPNAME = ANY(ARRAY['bpchar','varchar','varchar2','nvarchar2','nchar'])
             AND ATT.ATTTYPMOD > 4
    ...
    END::NUMBER AS CHAR_LENGTH,
    CASE
        WHEN T.TYPNAME = ANY(ARRAY['bpchar','char','nchar']) THEN 'B'
        ELSE NULL
    END::VARCHAR2(1) AS CHAR_USED,
    NULL::VARCHAR2(10) AS V80_FMT_IMAGE,
    NULL::VARCHAR2(10) AS DATA_UPGRADED,
    NULL::VARCHAR2(10) AS HISTOGRAM,
    NULL::NUMBER AS DEFAULT_LENGTH,
    NULL::NUMBER AS NULLABLE_COLUMN_ID,
    NULL::VARCHAR2(128) AS COLLATION_NAME,
    NULL::VARCHAR2(128) AS COLLATION_SCHEMA,
    NULL::VARCHAR2(128) AS COLLATION_OWNER,
    NULL::VARCHAR2(10) AS IDENTITY_COLUMN,
    NULL::VARCHAR2(10) AS SEGMENT_CREATED,
    NULL::VARCHAR2(10) AS LOB_FORMAT,
    NULL::NUMBER AS LOB_PRECISION,
    NULL::NUMBER AS LOB_LENGTH,
    NULL::VARCHAR2(10) AS NESTED,
    NULL::VARCHAR2(10) AS HIDDEN_COLUMN,
    NULL::VARCHAR2(10) AS VIRTUAL_COLUMN,
    NULL::NUMBER AS SEGMENT_ID,
    NULL::VARCHAR2(10) AS SENSITIVE_COLUMN
FROM PG_CLASS C
JOIN PG_NAMESPACE NS ON C.RELNAMESPACE = NS.OID
JOIN PG_ATTRIBUTE ATT ON C.OID = ATT.ATTRELID
