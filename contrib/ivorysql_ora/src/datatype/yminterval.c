 ...
 */
static void
EncodeYminterval(struct pg_tm *tm, fsec_t fsec, int style, char *str, Size bufsize, int precision)
{
	char	   *cp = str;
	int			year = tm->tm_year;
	int			mon = tm->tm_mon;
		...
		case INTSTYLE_SQL_STANDARD:
			{
				...
				else if (!sql_standard_value)
				{
					...
					cp += strlen(cp);
					AppendSeconds(cp, bufsize - (cp - str), sec, fsec, MAX_INTERVAL_PRECISION, true);
				}
