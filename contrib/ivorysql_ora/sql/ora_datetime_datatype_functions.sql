...
--
-- Resolve the overloads by OID instead of by pg_get_function_identity_arguments():
-- that function prints type names relative to search_path, so it reports
-- 'pg_catalog.int8' rather than 'bigint' under pg_regress' search path.
