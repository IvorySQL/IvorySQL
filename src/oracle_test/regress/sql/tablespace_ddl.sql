...
-- Oracle compatible mode converts '' to NULL (ivorysql.enable_emptystring_to_NULL),
-- which breaks both `LOCATION ''` (syntax error, NULL is not an Sconst) and option
-- values ''. Disable it so these tests exercise pg_get_tablespace_ddl() as in PG.
