...
-- Reject duplicate and conflicting SCALE/NOSCALE options.
CREATE SEQUENCE seq_dup_scale SCALE SCALE;
CREATE SEQUENCE seq_conflict_sn SCALE NOSCALE;
CREATE SEQUENCE seq_dup_noscale NOSCALE NOSCALE;
CREATE SEQUENCE seq_conflict_ns NOSCALE SCALE;

-- Reject duplicate and conflicting CACHE/NOCACHE options.
CREATE SEQUENCE seq_conflict_cn CACHE 5 NOCACHE;
CREATE SEQUENCE seq_conflict_nc NOCACHE CACHE 5;
CREATE SEQUENCE seq_dup_nocache NOCACHE NOCACHE;

-- An explicit NOCACHE on an Oracle-compatible identity column must not be
-- overridden by the default identity cache size of 20; an explicit CACHE size
-- must still be honored.
CREATE TABLE seq_ident_nocache (id number GENERATED BY DEFAULT AS IDENTITY (NOCACHE));
SELECT cache_size FROM all_sequences WHERE sequence_name = 'seq_ident_nocache_id_seq';
DROP TABLE seq_ident_nocache;
CREATE TABLE seq_ident_cache (id number GENERATED BY DEFAULT AS IDENTITY (CACHE 7));
SELECT cache_size FROM all_sequences WHERE sequence_name = 'seq_ident_cache_id_seq';
DROP TABLE seq_ident_cache;
