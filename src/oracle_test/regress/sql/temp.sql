...
-- Don't want cursor names and plisql function lines in the error messages
\set VERBOSITY terse
...
SELECT count(*), max(a) max_a, min(a) min_a, max(cnt) max_cnt FROM test_temp;
INSERT INTO test_temp(a, b, cnt) VALUES (-1, 'a', 0);
BEGIN;
INSERT INTO test_temp(a, b, cnt) VALUES (-2, 'a', 0);
DROP TABLE test_temp;
ROLLBACK;
SELECT count(*), max(a) max_a, min(a) min_a, max(cnt) max_cnt FROM test_temp;
