...
-- Oracle also allows a method specification to declare SELF explicitly.  The
-- declared parameter is the method's SELF argument, so the implicit SELF
-- argument must not be added a second time.
CREATE TYPE explicit_self_type AS OBJECT
(
    value integer,
    MEMBER PROCEDURE set_value(self IN OUT NOCOPY explicit_self_type,
                               p_value integer),
    MEMBER FUNCTION get_value(self IN explicit_self_type) RETURN integer
);
/
CREATE TYPE BODY explicit_self_type AS
    MEMBER PROCEDURE set_value(self IN OUT NOCOPY explicit_self_type,
                               p_value integer) IS
    BEGIN
        self.value := p_value;
    ...
    MEMBER FUNCTION get_value(self IN explicit_self_type) RETURN integer IS
    BEGIN
        RETURN self.value;
    END get_value;
END;
...
BEGIN
    v := explicit_self_type(1);
    v.set_value(42);
    IF v.get_value() <> 42 THEN
        RAISE EXCEPTION 'unexpected explicit SELF value %', v.get_value();
    END IF;
END;
/
DROP TYPE explicit_self_type;
