...
-- Use the same query in all the test cases below. (Oracle-compatible psql
-- intercepts a standalone EXECUTE as the SQL*Plus EXEC command, so the query
-- is written out inline at each viewpoint rather than being PREPAREd/EXECUTEd.)

-- viewpoint from regress_graph_rls_bob
SET SESSION AUTHORIZATION regress_graph_rls_bob;
SELECT * FROM GRAPH_TABLE (cabinet MATCH (u IS users)-[a IS accessed]->(d IS document) WHERE f_leak(d.dtitle) COLUMNS (u.pguser, a.aid, d.dtitle, d.dlevel)) ORDER BY 1, 2, 3, 4;
...
SET SESSION AUTHORIZATION regress_graph_rls_dave;
SELECT * FROM GRAPH_TABLE (cabinet MATCH (u IS users)-[a IS accessed]->(d IS document) WHERE f_leak(d.dtitle) COLUMNS (u.pguser, a.aid, d.dtitle, d.dlevel)) ORDER BY 1, 2, 3, 4; -- error
SET SESSION AUTHORIZATION regress_graph_rls_alice;
