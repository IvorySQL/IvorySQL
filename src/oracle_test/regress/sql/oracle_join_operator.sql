...
SELECT t2.c2, t1.c2
FROM t2, t1
WHERE t2.c3 = t1.c1(+) AND t1.c2(+) = 't1_a'
ORDER BY t2.c2;

SELECT t2.c2, t1.c2
FROM t2, t1
WHERE t2.c3 = t1.c1(+) AND t1.c2(+) IS NULL
ORDER BY t2.c2;
...
SELECT t2.c2, t1.c2
FROM t2, t1
WHERE t2.c2 = upper(t1.c2(+))
ORDER BY t2.c2;

SELECT t3.c3, t1.c2, t2.c2 FROM t1 JOIN t2 ON t1.c1 = t2.c3, t3 WHERE t2.c1 = t3.c2(+);
SELECT count(*) FROM (t1 CROSS JOIN t3), t2 WHERE t2.c3 = t1.c1(+);
SELECT t1.c2(+) FROM t1;
SELECT count(*) FROM t2, t1 WHERE t1.c1(+) IN (10, 20);
SELECT count(*) FROM t2, t1 WHERE t2.c3 = t1.c1(+) AND t1.c1(+) NOT BETWEEN 1 AND 5;
SELECT t1.c2, t2.c2, t3.c3 FROM t1, t2, t3 WHERE t1.c1 = t2.c3(+) AND t3.c1 = t2.c1(+) ORDER BY 1, 2, 3;
...
-- SELECT DISTINCT must honour (+): the DISTINCT grammar production used to
-- leave ora_join_op_exists unset, so the query silently ran as an inner
-- join. t2.c3 values 50 and NULL have no match in t1 and must still appear.
SELECT DISTINCT t2.c3
FROM t2, t1
WHERE t2.c3 = t1.c1(+)
ORDER BY t2.c3;

-- A (+) join predicate nested inside a parenthesised AND must not also be
-- left behind as a WHERE filter (which degraded the outer join to an inner
-- one). Row t2_d (c3 IS NULL) must survive, NULL-extended.
SELECT t2.c2 AS t2_c2, t1.c2 AS t1_c2
FROM t2, t1
WHERE t2.c1 <= 4 AND (t2.c3 = t1.c1(+) AND t2.c2 IS NOT NULL)
ORDER BY t2.c2;

-- "col(+) = constant" is an outer-join filter on the nullable table and
-- belongs in the ON clause, not in WHERE where it would drop the
-- NULL-extended rows.
SELECT t2.c2 AS t2_c2, t1.c2 AS t1_c2
FROM t2, t1
WHERE t2.c3 = t1.c1(+) AND t1.c2(+) = 't1_a'
...
-- "t.a(+) = t.b" references a single table: an outer-join filter, not a
-- self-join of the table with itself.
SELECT t2.c2 AS t2_c2, t1.c2 AS t1_c2
FROM t2, t1
WHERE t2.c3 = t1.c1(+) AND t1.c1(+) = t1.c1
ORDER BY t2.c2;

-- A (+) inside a subquery is that subquery's own business; the outer query
-- (which has no (+) of its own) must still run normally, not be dragged
-- into the rewrite. Inner query is t3 LEFT JOIN t1 on t3.c2 = t1.c1.
SELECT t2.c2
FROM t2
WHERE t2.c1 IN (SELECT t3.c1 FROM t3, t1 WHERE t3.c2 = t1.c1(+))
ORDER BY t2.c2;

-- Several one-table (+) filters on the same nullable relation all fold
-- into that outer join's ON clause.
SELECT t2.c2 AS t2_c2, t1.c2 AS t1_c2
FROM t2, t1
WHERE t2.c3 = t1.c1(+) AND t1.c2(+) = 't1_a' AND t1.c1(+) < 100
ORDER BY t2.c2;

-- (+) on a CTE column, together with a one-table (+) filter on the CTE.
WITH t1c AS (SELECT * FROM t1)
SELECT t2.c2 AS t2_c2, t1c.c2 AS t1c_c2
FROM t2, t1c
WHERE t2.c3 = t1c.c1(+) AND t1c.c2(+) = 't1_a'
ORDER BY t2.c2;

-- "(+ n)" is a parenthesised unary plus, not the outer-join operator; the
-- lexer's (+) lookahead must not disturb it.
SELECT (+ 1) AS a, (+t2.c1) AS b
FROM t2
WHERE t2.c1 = 2 AND (+1) = 1;

\set VERBOSITY terse
-- An unresolved (+) column must raise a clean error, not crash the backend.
SELECT * FROM t1, t2 WHERE t1.c1 = t2.nosuchcol(+);

-- A bare "(+)" that is not attached to a column is a syntax error.
SELECT t2.c2 FROM t2, t1 WHERE (+) = t1.c1;

-- "( + <non-')'>" is not the (+) operator: the "(" and "+" are handed
-- back and this parses as an ordinary parenthesised unary plus.
SELECT t2.c2 FROM t2, t1
WHERE t2.c3 = t1.c1 AND (+ t1.c1) = t1.c1
ORDER BY t2.c2;

-- (+) buried inside a larger expression is rejected, not silently ignored.
SELECT t2.c2, t1.c2 FROM t2, t1 WHERE t2.c3 = t1.c1(+) + 0 ORDER BY t2.c2;
SELECT t2.c2, t1.c2 FROM t2, t1 WHERE t2.c3 = abs(t1.c1(+)) ORDER BY t2.c2;

-- (+) is not allowed outside the WHERE clause of a SELECT.
DELETE FROM t2 WHERE t2.c1 = t2.c3(+);
UPDATE t2 SET c3 = 1 WHERE t2.c1 = t2.c3(+);

-- (+) may not be combined with explicit ANSI JOIN syntax.
SELECT t2.c2, t1.c2
FROM t2 JOIN t3 ON t2.c1 = t3.c1, t1
WHERE t2.c3 = t1.c1(+);

-- (+) in HAVING is rejected.
SELECT t2.c3, count(*)
FROM t2, t1
WHERE t2.c3 = t1.c1(+)
GROUP BY t2.c3
HAVING count(t1.c1(+)) > 0;
