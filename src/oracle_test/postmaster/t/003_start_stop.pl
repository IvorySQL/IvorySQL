...
# Restart server to ensure that backend launched for
# raw_connect_works() is gone. Otherwise, it might free up a
# connection slot later, when we expect all slots to be in use.
$node->restart;
