...
setup
{
	...
	-- IvorySQL Oracle compat: the test environment defaults to converting ''
	-- to NULL, which would turn the string_agg(chr(...), '') separator in the
	-- generator functions below into NULL and make them return NULL.  Disable
	-- it here so '' behaves as in PG.  (This runs in the global setup
	-- connection, before the CREATE FUNCTION statements below are parsed.)
