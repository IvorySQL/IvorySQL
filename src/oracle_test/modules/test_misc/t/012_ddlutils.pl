...
# IvorySQL: pin the server timezone to UTC so the VALID UNTIL timestamp
# below renders deterministically.  This mirrors the prior regression
# test's "SET timezone TO 'UTC'".  Without it the cluster inherits the
# machine default (the CI host is UTC+8), which shifts '2030-12-31
# 23:59:59+00' to the next day and breaks the VALID UNTIL assertions.
...
# (ivorysql.enable_emptystring_to_NULL), which makes LOCATION '' a syntax
# error.  Disable it so this in-place tablespace is created as in PG.
$node->safe_psql(
	'postgres', q{
