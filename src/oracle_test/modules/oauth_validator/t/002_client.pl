...
sub test
{
	...
	if (defined($params{log_like}))
	{
		...
		# postmaster to flush the log for finished connection.
		$node->wait_for_log(
			qr/DEBUG:  (?:00000: )?forked new client backend, pid=(\d+) socket.*DEBUG:  (?:00000: )?client backend \(PID \d\) exited with exit code \d/s,
			$log_start);
