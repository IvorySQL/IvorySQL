...
CREATE VIEW columns AS
    SELECT CAST(current_database() AS sql_identifier) AS table_catalog,
           ...
           CAST(CASE WHEN a.attidentity IN ('a', 'd', 'n', 'i', 'o') THEN 'YES' ELSE 'NO' END AS yes_or_no) AS is_identity, /* IvorySQL: sql-sequence */
           CAST(CASE WHEN a.attidentity IN ('a', 'i') THEN 'ALWAYS' WHEN a.attidentity IN ('d', 'o') THEN 'BY DEFAULT' END AS character_data) AS identity_generation, /* IvorySQL: sql-sequence */
