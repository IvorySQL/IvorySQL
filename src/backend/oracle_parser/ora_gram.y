...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	BINARY_FLOAT  BIT BOOLEAN_P BOTH BREADTH BY BYTE_P 
	...
	METHOD MINUTE_P MINVALUE MODE MODIFY MONTH_P MOVE 
...
%type <list>	ora_func_name opt_ora_func_args_with_defaults ora_createfunc_opt_list
				...
				accessor implementation_type implementation_package opt_procedure_args_with_defaults
				opt_ora_procedure_source_clause_list ora_procedure_source_clause_list
				opt_ora_createfunc_opt_list compiler_parameters_clause compiler_parameters_list
...
%token <keyword> EDITIONABLE NONEDITIONABLE IVYSQL NOCOPY SHARING METADATA AUTHID ACCESSIBLE
				 PACKAGE DETERMINISTIC RESULT_CACHE RELIES_ON USING_NLS_COMP SQL_MACRO SCALAR
 ...
 * IvorySQL create functions without parameters allow parentheses.
 */
 ...
 *
 *		QUERY:
 *				COMPATIBLE ORACLE MERGE COMMAND
 *
 *****************************************************************************/
			...
			| ConstInterval opt_interval_type	
				{
					...
					if (ORA_PARSER == compatible_db)
					{
						 ...
						 */
						if (n->val.val.ival == (INTERVAL_MASK(YEAR) | INTERVAL_MASK(MONTH)))
						{
							$$ = OracleSystemTypeName("yminterval");
							$$->typmods = $2;
							$$->location = @1;
						}
						else if (n->val.val.ival == (INTERVAL_MASK(DAY) | 
													 INTERVAL_MASK(HOUR) |
			...
			| CURRENT_DATE
				{
					...
					if (ORA_PARSER == compatible_db)
						$$ = (Node *) makeFuncCall(OracleSystemFuncName("current_date"), NIL, COERCE_EXPLICIT_CALL, @1);
					else
			...
			| CURRENT_TIMESTAMP
				{
					...
					if (ORA_PARSER == compatible_db)
						$$ = (Node *) makeFuncCall(OracleSystemFuncName("current_timestamp"), NIL, COERCE_EXPLICIT_CALL, @1);
					else
			...
			| CURRENT_TIMESTAMP '(' Iconst ')'
				{
					...
					if (ORA_PARSER == compatible_db)
						$$ = (Node *) makeFuncCall(OracleSystemFuncName("current_timestamp"), list_make1(makeIntConst($3, @3)), COERCE_EXPLICIT_CALL, @1);
					else
			...
			| LOCALTIMESTAMP
				{
					...
					if (ORA_PARSER == compatible_db)
						$$ = (Node *) makeFuncCall(OracleSystemFuncName("localtimestamp"), NIL, COERCE_EXPLICIT_CALL, @1);
					else
			...
			| LOCALTIMESTAMP '(' Iconst ')'
				{
					...
					if (ORA_PARSER == compatible_db)
						$$ = (Node *) makeFuncCall(OracleSystemFuncName("localtimestamp"), list_make1(makeIntConst($3, @3)), COERCE_EXPLICIT_CALL, @1);
					else
			...
			| ColId indirection
				{
					...
					else
					{
						...
						else
						{
							...
							crorfc->func = makeFuncCall(fname, NIL, COERCE_EXPLICIT_CALL, @1);
			...
			| ConstInterval Sconst opt_interval
				{
					...
					if (ORA_PARSER == compatible_db)
					{
						...
						n = linitial(typmods);
						if (n->val.val.ival == INTERVAL_MASK(YEAR) ||
							n->val.val.ival == INTERVAL_MASK(MONTH) ||
							n->val.val.ival == (INTERVAL_MASK(YEAR) | INTERVAL_MASK(MONTH)))
						{
