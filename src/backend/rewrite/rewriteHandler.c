					...
					Bitmapset **unused_values_attrnos)
{
	...
	for (attrno = 1; attrno <= numattrs; attrno++)
	{
		...
		if (commandType == CMD_INSERT)
		{
			...
			if (att_tup->attidentity == ATTRIBUTE_ORA_IDENTITY_ALWAYS && !apply_default)
			{
				/*
				 * Test whether this column's VALUES-RTE entries are all
				 * SetToDefault (same as the standard ALWAYS branch above).
				 */
				if (values_attrno != 0)
				{
					if (default_only_cols == NULL)
				...
				if (!apply_default)
					ereport(ERROR,
							(errcode(ERRCODE_GENERATED_ALWAYS),
							errmsg("cannot insert a generated always identity column \"%s\"",
									NameStr(att_tup->attname)),
							errdetail("Column \"%s\" is an identity column defined as GENERATED ALWAYS.",
									NameStr(att_tup->attname))));
			}
			if (att_tup->attidentity == ATTRIBUTE_ORA_IDENTITY_BY_DEFAULT && new_tle &&
				(IsA(new_tle->expr, Const) || IsA(new_tle->expr, FuncExpr)))
			{
				if (IsA(new_tle->expr, Const))
				{
					...
					if (var->constisnull)
						ereport(ERROR,
								(errcode(ERRCODE_GENERATED_ALWAYS),
							errmsg("cannot insert NULL into ( \"%s\".\"%s\" )",
									 NameStr(target_relation->rd_rel->relname), NameStr(att_tup->attname)),
							errdetail("Column \"%s\" is an identity column defined as GENERATED BY DEFAULT.",
									NameStr(att_tup->attname))));
				}
		...
		if (att_tup->attidentity == ATTRIBUTE_IDENTITY_DEFAULT_ON_NULL && new_tle && IsA(new_tle->expr, Const))
		{
			Const   *var = (Const *) new_tle->expr;
			if (var->constisnull)
				apply_default = true;
		...
		{
			if (att_tup->attidentity == ATTRIBUTE_ORA_IDENTITY_ALWAYS && new_tle && !apply_default)
				ereport(ERROR,
						(errcode(ERRCODE_GENERATED_ALWAYS),
						 errmsg("cannot update a generated always identity column \"%s\"",
									NameStr(att_tup->attname)),
						 errdetail("Column \"%s\" is an identity column defined as GENERATED ALWAYS.",
									NameStr(att_tup->attname))));

			if ((att_tup->attidentity == ATTRIBUTE_ORA_IDENTITY_BY_DEFAULT)
					&& new_tle && IsA(new_tle->expr, Const))
			{
				Const   *var = (Const *) new_tle->expr;
				if (var->constisnull)
					ereport(ERROR,
						(errcode(ERRCODE_GENERATED_ALWAYS),
						 errmsg("cannot update (\"IDENTITY_DEFAULT\".\"%s\") to null",
									NameStr(att_tup->attname)),
						 errdetail("Column \"%s\" is an identity column defined as BY DEFAULT",
									NameStr(att_tup->attname))));
			}
