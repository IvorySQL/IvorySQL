					   ...
					   int reqnatts)
{
	...
	if (HeapTupleHasNulls(tuple))
	{
		...
		tp = (char *) tup + MAXALIGN(offsetof(HeapTupleHeaderData, t_bits) +
									 BITMAPLEN(natts) +
									 ((tup->t_infomask & HEAP_HASROWID) ? sizeof(int64) : 0));
...
ExecTypeFromTLInternal(List *targetList, bool hasrowid, bool skipjunk)
{
	...
	foreach(l, targetList)
	{
		...
		/*
		 * If a function which has out parameters is called in SQL, not in PLSQL, 
		 * the return type should be modified to be record.
		 */
