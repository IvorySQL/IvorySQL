...
kwlist_line: while (<$kwlist>)
{
	...
	if ($line =~ /^PG_KEYWORD\(\"(.*)\", (.*), (.*), (.*)\)/)
	{
		...
		if ($kwstring !~ /^[a-z_]+$/ and $kwstring ne "varchar2" and $kwstring ne "nvl2") # IvorySQL: datatype
		{
			error
		...
		if ($kwname !~ /^[A-Z_]+$/ and $kwname ne "VARCHAR2" and $kwname ne "NVL2")	# IvorySQL: datatype
		{
			error
