...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	RANGE READ REAL REASSIGN RECHECK RECORD RECURSIVE REF_P REFERENCES REFERENCING
...
package_refcursor:
		TYPE_P any_name IS REF_P CURSOR			/* Weakly Typed Ref_p Cursor */
			{
		...
		| TYPE_P any_name IS REF_P CURSOR RETURN
			type_function_name '%' ROWTYPE_P 	/* Strongly Typed Ref_p Cursor */
			{
				CreateDomainStmt *refc = makeNode(CreateDomainStmt);
				refc->domainname = $2;
				refc->typeName = typeStringToTypeName("refcursor");
