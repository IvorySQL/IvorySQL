 ...
 *
 * src/interfaces/libpq/ivytest/testlibpq_number.c
 *
 *-------------------------------------------------------------------------
 */
...
char *function_str = "create or replace function test_number(id integer,number1 out number, number2 out number[],number3 number) return number as "
"  mds number(4,2)[3];"
" begin"
"     mds[0] := 11.22;"
"     mds[1] := 33.44;"
...
exec_prepare(Ivyconn *conn)
{
	...
	const char *stmt = "select * from test_number($1,$2,$3,$4);";
	Oid param_types[4];
	IvyPreparedStatement *stmthandle;
	const char* custid = "200";
	const char* time1 = "24.99";
	const char* time2 = "25.99'";
	const char* time3 = "26.99";
	...
	param_types[1] = 9520 | 0x40000000  /* out */;
	param_types[2] = 9521 | 0x40000000  /* out */;
	param_types[3] = 9520 | 0x20000000  /* in */;
...
main()
{
	...
	res = Ivyexec(conn, "drop function test_number(integer, number, number[], number);");
	Ivyclear(res);
