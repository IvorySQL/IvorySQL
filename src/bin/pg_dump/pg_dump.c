...
dumpTableSchema(Archive *fout, const TableInfo *tbinfo)
{
	...
	char	   *qualrelname;
	int			numParents;
	bool		is_ora_identity = false;	/* IvorySQL:sql-sequence */
	TableInfo **parents;
			...
			for (j = 0; j < tbinfo->numatts; j++)
			{
				...
				if (shouldPrintColumn(dopt, tbinfo, j))
				{
					...
					print_default = ((tbinfo->attrdefs[j] != NULL &&
									 ...
									 !tbinfo->attrdefs[j]->separate) || is_ora_identity); 	/* IvorySQL:sql-sequence */
					...
					if (print_notnull&& !is_ora_identity)	/* IvorySQL:sql-sequence */
						appendPQExpBufferStr(q, " NOT NULL");
					...
					is_ora_identity = false;	/* IvorySQL:sql-sequence */
				}
			}
...
dumpSequence(Archive *fout, const TableInfo *tbinfo)
{
	...
	else if (!is_ora_identity)	/* IvorySQL:sql-sequence */
