...
/* flex reentrant accessors (generated by psqlplusscan.l, no header exported) */
extern char *orapsql_yyget_text(yyscan_t yyscanner);
extern int orapsql_yyget_leng(yyscan_t yyscanner);
...
exec_stmt: exec_keyword
		{
			...
			if (state->is_sqlplus_cmd)
			{
				...
				int	offset = (orapsql_yyget_text(yyscanner) - state->scanbuf)
					+ orapsql_yyget_leng(yyscanner);
